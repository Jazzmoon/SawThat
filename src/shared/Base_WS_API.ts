...
/**
 * class that wraps websocket to facilitate communication with the server
...
export default class Base_WS_API {
  ...
  > = {};
  private static incomingMessageCallbacks: Record<
    string,
  ...
  /**
   * We use user tokens to identify users over WebSockets. This method lets you set the token (which the server will send to the node)
   ...
   */
  public static addIncomingMessageCallback(
    id: string,
    incomingMessageCallback: (data: WebsocketMessage) => void
  ): void {
   ...
   */
  protected static async setupWebSocketConnection(
    wsURL: string
  ): Promise<boolean> {
    ...
    Base_WS_API.socket.onopen = (event: Event) => {
      ...
      delete Base_WS_API.pendingRequests[requestId];
    };
  ...
  protected static async sendRequest(
    ...
    if (
      Base_WS_API.socket == null ||
      Base_WS_API.socket.readyState !== WebSocket.OPEN
    ) {
      return {
        type: WebsocketType.Error,
        requestId: requestId,
   ...
   */
  private static addRequestToQueue(
    requestId: string
  ): Promise<WebsocketMessage> {
  ...
  public static sendMessageToCallbacks(data: WebsocketMessage): void {
    for (const callback of Object.values(
      Base_WS_API.incomingMessageCallbacks
    )) {
      callback(data);
    }
  }
